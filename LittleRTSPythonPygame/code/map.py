...
class Map:
    ...
    def draw(self, surface, teams):
        surface.blit(self.image, ACTWINDPOS, ((self.scrollx, self.scrolly), ACTWINDSIZE))        
        self.drawminimap(teams)
        surface.blit(self.minimap, MINIMAPOS)
        
    def clean(self):
        self.image.blit(self.emptyimage, (self.scrollx, self.scrolly), ((0, 0), ACTWINDSIZE))
        
    def scroll(self, coord):
        mousex, mousey = coord
        if ((mousex >  GUISIZE[0] - 5) and 
        (self.scrollx + ACTWINDSIZE[0] < SIZEMAP[0])):
            self.scrollx += 15
        if ((mousey >  GUISIZE[1] - 5) and 
        (self.scrolly + ACTWINDSIZE[1] < SIZEMAP[1])):
            self.scrolly += 15
        if ((mousex < GUIPOS[0] + 15) and 
            (self.scrollx > 0)):
            self.scrollx -= 15
        if ((mousey < GUIPOS[1] + 15) and 
            (self.scrolly > 0)):
            self.scrolly -= 15
    ...
    def drawminimap(self, teams):
        ...
        pos = (MINIMASIZE[0] * self.scrollx // SIZEMAP[0],
    ...
    def mouseovermap(self, coord):
        x, y = coord
        toret = (x + self.scrollx - ACTWINDPOS[0], 
                 y + self.scrolly - ACTWINDPOS[1])
        return toret
