...
class Interface:
    def __init__(self):
        ...
        self.commbuttons = {
            'move':{
                0:load_image(PATHS['menus'] + 'commgoto.png', True)
            },
            'passive':{
            ...
            'scout':{
                0:load_image(PATHS['menus'] + 'perdix.png', True)
            },
            'square':{
                0:load_image(PATHS['menus'] + 'square.png', True)
            },
            'star':{
                0:load_image(PATHS['menus'] + 'star.png', True)
            },
            'gunner':{
                0:load_image(PATHS['menus'] + 'gunner.png', True)
            },
            'offensive':{
                0:load_image(PATHS['menus'] + 'offensive.png', True)
            }
        }
        self.buttonrects = {
        ...
        self.helpinfo = {
            ...
            'aggressive':self.font10.render('Cambiar a actitud agresiva', False, WHITE),
    ...
    def drawinfounit(self, unit):
        ...
        life = self.font10.render(str(unit.life) + '-' + 
                           str(unit.lifemax), False, WHITE)
        ...
        pygame.draw.rect(self.backinfo[0], GREEN, ((7, 20), (name.get_width() + 2, 7)), 1)
    ...
    def drawinfoplanet(self, planet):
        ...
        if planet.color == 'c':
            if planet.settler > 0:
                settler = self.font12.render('Red Team: ' + str(planet.settler), False, RED)
            elif planet.settler < 0:
                settler = self.font12.render('Blue Team: ' + str(planet.settler * -1), False, BLUE)
            else:
    ...
    def drawinfodefense(self, defense):
        ...
        life = self.font10.render(str(defense.life) + '-' + 
                           str(defense.lifemax), False, WHITE)
        armor = self.font12.render('Armor: ' + str(defense.armor)
                              , False, WHITE)
        damage = self.font12.render('Damage: ' + str(defense.attack)
                              , False, WHITE)
        sight = self.font12.render('Sight: ' + str(defense.view)
                              , False, WHITE)
        ...
        rectlevel.fill(WHITE)
        self.backinfo[0].blit(name, (7, 6))
        pygame.draw.rect(self.backinfo[0], GREEN, 
                         ((7, 20), (name.get_width() + 2, 7)), 1)
        self.backinfo[0].blit(rectlife, (8, 21))
        self.backinfo[0].blit(life, (13, 27))
        self.backinfo[0].blit(armor, (75, 50))
        self.backinfo[0].blit(damage, (75, 65))
    ...
    def drawresource(self):
        ...
        if self.teams[0].pob >= self.teams[0].maxpob:
            pob = self.font12.render((str(self.teams[0].pob) + '-' +
                                      str(self.teams[0].maxpob)), False, RED)
        else:
            pob = self.font12.render((str(self.teams[0].pob) + '-' +
                                      str(self.teams[0].maxpob)), False, WHITE)
        ...
        land = self.font12.render((str(self.teams[0].base.landings[0]) + '-' +
                                   str(self.teams[0].base.landings[1])), False, WHITE)
        ...
        for i in self.teams[1].shield:
            ...
            if i.percent() > 70:
                ...
                rectlife.fill((139, 0, 0))
            self.backresor[0].blit(rectlife, pos.pop(0))
            land = self.font12.render((str(self.teams[1].base.landings[0]) + '-' +
                                       str(self.teams[1].base.landings[1])), False, WHITE)
