...
class Team(pygame.sprite.LayeredDirty):
    ...
    def append(self, unit, coord=None):
        if (self.pob < self.maxpob and
            self.resources - self.costs[unit] >= 0):
    ...
    def update(self):
        ...
        if self.contaddresources == 50:
            self.contaddresources = 0
            self.resources += 150
            self.resources = self.maxresources if self.resources > self.maxresources else self.resources   
        deadsprites  = []
        contdefenses = 0
        contplanets  = 0
        ...
        for i in self.sprites():
            ...
            if i.aggressive and i.target == None:
                for j in enemyteam.sprites():
                    if (j.type != 'planet' and j.type != 'bullet' and 
                        distance(i.rect, j.rect) <= i.view * 100):
                        i.target = j
                        break
    ...
    def supr(self):
        if self.selectedunit:
            if (self.selectedunit.type == 'unit' or 
                self.selectedunit.type == 'settler'):
                playsound('deadunit')
                self.selectedunit.life = 0
    ...
    def automata(self, enemyteam, neutralteam):
        if self.reaction >= 100:
            ...
            if self.pob == self.maxpob:
                for unit in self.sprites():
                    if (unit.type != 'settler' and
                        unit.type != 'planet'  and
                        unit.type != 'bullet'  and
                        unit.type != 'defense'):
                        unit.life = 0
                        break          
